...
def simple2tradition(line):
    # convert simplify chinese to traditional chinese
    if isinstance(line, list):      # line consists of tokens
        line = [Converter('zh-hant').convert(tok) for tok in line]
    elif isinstance(line, str):
        line = Converter('zh-hant').convert(line)
    else:
        raise ValueError(f"Unsupported input type: {type(line)}")
    return line


def tradition2simple(line):
    # convert traditional chinese to simplify chinese
    if isinstance(line, list):      # line consists of tokens
        line = [Converter('zh-hans').convert(tok) for tok in line]
    elif isinstance(line, str):
...
if __name__ == '__main__':
    print(tradition2simple("拉面師傅給我們做了許多好吃的拉面。"))
