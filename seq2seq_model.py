...
class Seq2SeqModel:
    ...
    def predict_on_test(self):
        ...
        def prediction(model, input_txt, category2template, tokenizer, is_chinese, device):
            ...
            entity_list = []
            span_max_len = self.hyper_parameters.span_max_len + int(len(input_txt_list) * self.hyper_parameters.span_alpha)
            ...
            for ith, word_spans in enumerate(candidate_spans):
                if len(word_spans) > 0:
                    ...
                    entity = template_entity(model, word_spans, input_txt, ith, category2template, tokenizer,
                                             is_chinese,
                                             device)
        ...
        is_chinese = True if self.hyper_parameters.dataset in ['msra', 'ontonotes4', 'resume', 'weibo'] else False
        category2template = dataset_category2template[self.hyper_parameters.dataset]
        if is_chinese:
            tokenizer = BertTokenizer.from_pretrained(self.hyper_parameters.pretrain_path)
        else:
            tokenizer = BartTokenizer.from_pretrained(self.hyper_parameters.pretrain_path)
        model = BartForConditionalGeneration.from_pretrained(self.hyper_parameters.checkpoint)
        model.eval()
        model.config.use_cache = False
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        model.to(device)

        file_path = f'./data/{self.hyper_parameters.dataset}/test.txt'
        examples = []
